...
<div class="fileinput fileinput-new" data-provides="fileinput">
    <div style="margin-bottom:5px;">
        <span class="btn red btn-outline btn-file">
            ...
            <input type="file" name="..." class="file_buttom<?=$saveDB?>" accept="image/gif,image/jpeg,image/jpg,image/png,image/svg">
        </span>
    </div>
    <div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 200px; height: 150px;">
        <img class="file_img" src="<?= !empty($picture['path']) ? $domain.\common\helpers\Html::src($picture['path']) : \yii\helpers\Url::to('@web/static/img/no.png'); ?>">
    </div>
</div>
...
$(function() {
    ...
    $(".file_buttom<?=$saveDB?>").on("change", function(){
        var file_ipt = $(this).parent().siblings('input');
            ...
            reader.onloadend = function(){
                $.ajax({
                    ...
                    success: function(json){
                        if(json.boo){
                            file_img.attr('src','<?=Yii::$app->params['image']?>'+json.data.url);
