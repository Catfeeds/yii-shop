...
use common\models\shop\CouponAutoSend;
use common\models\shop\Form;
...
class OrderSubmitPreviewForm extends Model
{
    ...
    private function getDataByCartIdList($cart_id_list, $store)
    {
        ...
        foreach ($cart_list as $item) {
            ...
            if($goods->is_product && $item->product_id)
            {
...
//                    $new_item->give = ($new_item->price * ($give/100)) * $store->integral;
        ...
        $address = Address::find()->select(['_id','name','mobile','province_id','province','city_id','city','district_id','district','detail','is_default'])->where([
            '_id' => $this->address_id,
            'user_id' => $this->user_id,
        ])->asArray()->one();
        if (!$address) {
            $address = Address::find()->select(['_id','name','mobile','province_id','province','city_id','city','district_id','district','detail','is_default'])->where([
                'user_id' => $this->user_id,
            ])->orderBy('is_default DESC,created_at DESC')->asArray()->one();
        }
        $express_price = 0;
        if ($address) {
