...
function getData(offset,size){
	$.ajax({
		...
		success: function(reponse){
			...
			for(var i=0; i< sum; i++){
				result += '<div class="item"><a class="link_pic" href="'+data[i].url+'"><img src="'+data[i].thumb+'"></a><span>'+data[i].title+'</span><a class="link_p" href="'+data[i].url+'">'+data[i].summary+'</a></div>'
			}
